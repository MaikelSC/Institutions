...
<title><?=isset($general)?'Editar Elemento':'Adicionar Elemento'?></title>
<!-- InstanceEndEditable -->
...
<script type='text/javascript' src='<?=base_url()?>application/views/js/general.js'></script>
...
<table  border="0" id="cuerpo" >	
  ...
  <tr><td colspan="2"> <div id="tbodyAdmin">
		  ...
		  <tr>
			 ...
			 <div >
		<form method="post" id="formAddGen" action="<?=isset($general)?site_url("c_general/EditarGeneral/".$general->id_general):site_url("c_general/AdicionarGeneral")?>" onsubmit="return validarFormulario(this.id)" />
		<table id="tablaForm">
			<tr>
				<td colspan="4" class="tituloForm" ><?=isset($general)?'Editar general':'Adicionar general'?></td>
			</tr>
			<tr>
				<td class="label" width="15%">Nombre:</td>
				<td width="50%" class="labelCampo"><input type="text" id="nombreUniv" name="nombre" class="textbox obligatorio" value="<?=isset($general)?$general->nombre:""?>"/></td>
				<td></td>
				<td ></td>
			</tr>	
			...
			<tr>
				...
				<textarea cols="20" rows="8" id="textAGener" class="obligatorio" name="cuerpo"><?=isset($general)?$general->valor:""?></textarea></div></td>
			</tr>
			<tr>
				<td colspan="4" ><div>
				<input type="submit" class="botonCont" style="background-color:#666666;margin-left:5px;" value="<?=isset($general)?"Guardar":"Adicionar"?>" name="<?=isset($general)?"Editar_General":"Adicionar_General"?>" />
				</div></td>
			</tr>		
		</table>
		</form>
